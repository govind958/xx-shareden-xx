...
// Initialize Mixpanel (do this once, maybe in _app.tsx instead)
mixpanel.init("YOUR_MIXPANEL_TOKEN");

const App: FC = () => {
  const [isSheetOpen, setIsSheetOpen] = useState<boolean>(false);
  const [visitStartTime] = useState<number>(Date.now());
  ...
  useEffect(() => {
    mixpanel.track("Page Viewed", {
      ...
      country: "unknown", // can integrate geo API if needed
      city: "unknown",
    });
  }, []);

  // Utility: get UTM query param
      ...
      <section className="flex-grow container mx-auto px-4 md:px-8 lg:px-12 py-16 md:py-24 grid md:grid-cols-2 gap-8 md:gap-16 items-center z-10 relative">
        <div className="flex flex-col items-center md:items-start text-center md:text-left animate-fade-in-up">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-snug mb-6 max-w-2xl">
            Rent the <span className="text-white underline decoration-teal-500">Stack</span> Your Startup Needs
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <Button size="lg" className="bg-white text-neutral-900 font-semibold hover:bg-neutral-200 transition" onClick={() => handleCTAClick("Discover Stacks Hero", "/discover")}>
              Discover Stacks
            </Button>
            <Button size="lg" className="border border-neutral-500 text-neutral-200 hover:border-white hover:text-white transition" onClick={() => handleCTAClick("Login Hero", "/login")}>
              Login
            </Button>
          </div>
        </div>
