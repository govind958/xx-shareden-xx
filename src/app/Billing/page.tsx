...
const BillingPage: FC = () => {
  ...
  const [selectedOrderItemToCancel, setSelectedOrderItemToCancel] = useState<string>('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (stacks.length > 0 && !selectedOrderItemToCancel) {
          setSelectedOrderItemToCancel(stacks[0].id);
        }
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      const result = await cancelSubscription(selectedOrderItemToCancel, user.id);
          ...
          <div className="space-y-6 max-w-4xl">
              ...
              <input
                ...
                onChange={() => {
                  ...
                  if (purchasedStacks.length > 0) {
                    setSelectedOrderItemToCancel(purchasedStacks[0].id);
                  }
                }}
                ...
                <label className="flex gap-6 cursor-pointer">
                  <input
                    ...
                    onChange={() => {
                      ...
                      if (purchasedStacks.length > 0 && !selectedOrderItemToCancel) {
                        setSelectedOrderItemToCancel(purchasedStacks[0].id);
                      }
                    }}
                    className="mt-1 w-5 h-5 accent-teal-500"
                      ...
                      {purchasedOrders.length === 0 ? (
                        ...
                        purchasedStacks.map((stack) => (
                          <tr key={stack.id} className="group hover:bg-white/[0.02]">
                            <td className="px-6 py-5">
                              <div className="flex items-start gap-4">
                                <input
                                  ...
                                  name="stack-select"
                                  value={stack.id}
                                  checked={selectedOrderItemToCancel === stack.id}
                                  onChange={(e) => setSelectedOrderItemToCancel(e.target.value)}
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="text-[10px] text-zinc-600 font-mono">ID: {stack.id.slice(0, 8)}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-5">
                            ...
                            <td className="px-6 py-5">
                              <span className="text-xs text-zinc-400 capitalize">{stack.status}</span>
                            </td>
                            <td className="px-6 py-5">
                              <div className="text-[10px] text-zinc-600 font-mono">#{stack.order_id.slice(0, 8).toUpperCase()}</div>
                            </td>
                          </tr>
                        ))
                      )}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-white/5">
              <Button
                ...
                disabled={isCancelling || !selectedOrderItemToCancel}
