    ...
    <div className="flex justify-between items-start mb-6">
      ...
      <div className="flex flex-col items-end gap-1">
        <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 ${
          status === 'processing' ? 'bg-teal-500/10 text-teal-500' : 'bg-orange-500/10 text-orange-500'
        }`}>
          {status}
        </span>
        {!isActive && (
...
export default function ProjectPage() {
  ...
  useEffect(() => {
    async function fetchAssignedTasks() {
      ...
      if (user) {
        // 2. Fetch order_items assigned to this user. 
        // FIX: Removed .eq('is_active', true) so that Govind's tasks show even if is_active is false.
        const { data, error } = await supabase
          ...
          .select(`
            ...
            is_active
          `)
          .eq('assigned_to', user.id); // Filtering by Govind's ID
